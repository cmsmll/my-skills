...
async function main() {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (a) {
      ...
      case "-h": case "--help": await say("用法: node remove.mjs --name <供应商名> [-f models.yml] | --list"); break;
      default: opts.name = a;
    }
  }
  ...
  if (!opts.name) {
    await say("✗ 需要 --name <供应商名>（或 --list 查看列表）");
    if (typeof process !== "undefined") process.exitCode = 1;
    return;
  }
  ...
  await say(`  剩余供应商: ${providers.filter((p) => p.name !== opts.name).map((p) => p.name).join(", ") || "无"}`);
  await say(`  重开会话后生效`);
}
